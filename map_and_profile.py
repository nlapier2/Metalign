...
from cli_validation import check_minimap2_bin
...
def filter_line(args, splits):
    ...
    for ch in cigar:
        if not ch.isalpha():  # ch is a number
            ...
            total_len += cur
            cur = 0
    # TODO: var edit_distance not used (peterk87: Wed Jan 22 10:14:01 CST 2020)
...
def tree_results_cami(args, taxids2abs):
    ...
    Args:
        args (argparse.Namespace): argparse.Namespace containing all command-line args and options
        taxids2abs (dict): taxid to abundance info

    Returns:
        Dict of clade taxid to list containing taxid, rank, taxid lineage, sci name lineage, abundance
    """
...
def compute_abundances(args, infile, acc2info, tax2info, minimap_bin='minimap2'):
    """Processes uniquely-mapped reads, then estimates abundances using uniquely-mapped abundances and multimapped read information
    ...
    Args:
        ...
        acc2info (dict): accession to list of info about accession
        tax2info (dict): taxid to list of info about taxid
    ...
    if args.input_type == 'sam':  # input stream from sam file
        ...
        cmd = [minimap_bin,
               ...
               args.db,
               infile]
        logging.info('Running minimap2 command: ' + ' '.join(cmd))
        proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, bufsize=1)
        instream = iter(proc.stdout.readline, "")
